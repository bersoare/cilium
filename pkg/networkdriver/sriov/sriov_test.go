...
func testLogger(t testing.TB) *slog.Logger {
	return hivetest.Logger(t)
}

// ----------------------------------------------------------------------------
// sriovCfg — fluent builder for SRIOVDeviceManagerConfig used in tests.
// ----------------------------------------------------------------------------

type sriovCfg struct {
	ifaces []v2alpha1.SRIOVDeviceConfig
}

func newSRIOVCfg() *sriovCfg { return &sriovCfg{} }

func (c *sriovCfg) withIface(ifname string, vfCount int) *sriovCfg {
	c.ifaces = append(c.ifaces, v2alpha1.SRIOVDeviceConfig{IfName: ifname, VfCount: vfCount})
	return c
}

func (c *sriovCfg) build() *v2alpha1.SRIOVDeviceManagerConfig {
	return &v2alpha1.SRIOVDeviceManagerConfig{
		Enabled: true,
		Ifaces:  c.ifaces,
	}
}
...
//
// Tests construct it via newFakeNetlink(), register links with addLink(), and
// inspect recorded calls via the Calls slice after the fact.
// ----------------------------------------------------------------------------

type netlinkCall struct {
	Op   string // "LinkByName" | "LinkSetVfVlan"
	Name string // for LinkByName
	Link netlink.Link
	VF   int
	VLAN int
}
...
type fakeNetlinkOps struct {
	...
	links []netlink.Link
	// Calls records every mutating call made; tests inspect this to assert behaviour.
	Calls []netlinkCall
	// linkSetVfVlanErr, if non-nil, is returned by LinkSetVfVlan.
	linkSetVfVlanErr error
	// linkByNameErr, if non-nil, is returned by LinkByName for any name.
	linkByNameErr error
}
...
func (f *fakeNetlinkOps) LinkSetVfVlan(link netlink.Link, vf, vlan int) error {
	f.Calls = append(f.Calls, netlinkCall{Op: "LinkSetVfVlan", Link: link, VF: vf, VLAN: vlan})
...
func TestSriov(t *testing.T) {
	...
	t.Run("setup on startup", func(t *testing.T) {
		fs, nl := setupFakes(t)
		cfg := newSRIOVCfg().withIface("mypf", 1).build()

		// NewManager calls init() → setupVfs(), which writes sriov_numvfs=1.
		mgr, err := NewManager(testLogger(t), cfg, withNetlinkOps(nl), withSysPath(fs.root))

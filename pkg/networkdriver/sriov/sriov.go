...
// realNetlinkOps delegates to the production safenetlink/netlink implementations.
type realNetlinkOps struct{}

func (realNetlinkOps) LinkList() ([]netlink.Link, error) {
...
func (realNetlinkOps) LinkSetVfVlan(link netlink.Link, vf, vlan int) error {
...
func (d PciDevice) Setup(config types.DeviceConfig) error {
	...
	if config.Vlan != 0 {
		return d.nl.LinkSetVfVlan(l, d.VfID, int(config.Vlan))
	}
...
func (d PciDevice) Free(config types.DeviceConfig) error {
	...
	if config.Vlan != 0 {
		return d.nl.LinkSetVfVlan(l, d.VfID, 0)
	}
